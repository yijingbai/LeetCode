...
if [ $# -gt 1 ] && [ -f $2 ]; then
    ...
    source_folder=`echo $source_file | awk -F '-' '{for (i=1; i<=NF; i++) printf("%s", toupper(substr($i,1,1)) substr($i,2)) }'`
    source_file=${source_folder}${FILE_EXT}
...
echo "######Tag: \`HashMap\`" >> ${source_folder}/README.md
echo "######Difficulty: \`Hard\`" >> ${source_folder}/README.md
mv ${source_folder} LinkedList
