 ...
 * secret number exactly in both digit and position (called "bulls") and how many
...
public:
    string getHint(string secret, string guess) {
        ...
        for (int i = 0; i < secret.size(); i++) {
            int secret_v = secret[i] - '0';
            ++digit_map[secret_v];
        }

        for (int i = 0; i < guess.size(); i++) {
            int secret_v = secret[i] - '0';
            int guess_v  = guess[i] - '0';
            if (secret_v == guess_v) {
                ++bull;
                --digit_map[guess_v];
            }
        ...
        for (int i = 0; i < guess.size(); i++) {
            int secret_v = secret[i] - '0';
            int guess_v  = guess[i] - '0';
            if (secret_v != guess_v && digit_map[guess_v] > 0) {
                ++cow;
                --digit_map[guess_v];
            }
        }
        ...
        string result;
        result += to_string(bull) + "A";
        result += to_string(cow) + "B";
        return result;
    }
};
